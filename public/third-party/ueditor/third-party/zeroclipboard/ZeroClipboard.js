...
(function(window) {
    ...
    var _swfPath = (function() {
        ...
        if (
            !(document.currentScript && (jsPath = document.currentScript.src))
        ) {
            ...
            if ("readyState" in scripts[0]) {
                ...
                for (i = scripts.length; i--; ) {
                    ...
                    tmpJsPath = tmpJsPath.slice(
                        0,
                        tmpJsPath.lastIndexOf("/") + 1
                    );
                    if (jsDir == null) {
                        jsDir = tmpJsPath;
    ...
    var _getDOMObjectPosition = function(obj, defaultZIndex) {
        ...
        if (obj.getBoundingClientRect) {
            ...
            info.height =
                "height" in rect ? rect.height : rect.bottom - rect.top;
        }
        return info;
    };
    var _cacheBust = function(path, options) {
        var cacheBust =
            options == null || (options && options.cacheBust === true);
        if (cacheBust) {
            return (
                (path.indexOf("?") === -1 ? "?" : "&") +
                "noCache=" +
    ...
    var _vars = function(options) {
        ...
        if (trustedOriginsExpanded.length) {
            str +=
                "trustedOrigins=" +
                encodeURIComponent(trustedOriginsExpanded.join(","));
        }
            ...
            if (clipResults.hasOwnProperty(prop)) {
                ...
                for (var dataFormat in tmpHash) {
                    if (
                        ...
                        newResults[prop][formatMap[dataFormat]] =
                            tmpHash[dataFormat];
                    }
                }
            }
    ...
    ZeroClipboard.clearData = function(format) {
        if (typeof format === "undefined") {
            ...
            _clipData.hasOwnProperty(format)
        ) {
    ...
    var _bridge = function() {
        ...
        if (!container) {
            ...
            var allowNetworking =
                allowScriptAccess === "never" ? "none" : "all";
            var flashvars = _vars(_globalConfig);
            var swfUrl =
                _globalConfig.swfPath +
                _cacheBust(_globalConfig.swfPath, _globalConfig);
    ...
    var _createEvent = function(eventType, event) {
        ...
        if (event.type === "error") {
            event.target = null;
            if (
                /^flash-(outdated|unavailable|deactivated|overdue)$/.test(
                    event.name
                )
            ) {
                _extend(event, {
    ...
    var _getRelatedTarget = function(targetEl) {
        ...
        return relatedTargetId
            ? document.getElementById(relatedTargetId)
            : null;
    };
        ...
        for (var i = 0; i < elements.length; i++) {
            if (
                ...
                if (!elements[i].zcClippingId) {
                    elements[i].zcClippingId =
                        "zcClippingId_" + _elementIdCounter++;
                    ...
                    _inArray(
                        this.id,
                        _elementMeta[elements[i].zcClippingId]
                    ) === -1
                ) {
