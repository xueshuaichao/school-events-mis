...
var CodeMirror = (function() {
    ...
    function CodeMirror(place, givenOptions) {
            ...
            addWidget: function(pos, node, scroll, vert, horiz) {
                ...
                if (vert == "over") top = pos.y;
                    ...
                    var vspace = Math.max(
                        ...
                        hspace =
                            Math.max(code.clientWidth, lineSpace.clientWidth) -
                            paddingLeft();
                    if (
        ...
        function updateLinesNoUndo(from, to, newText, selFrom, selTo) {
            ...
            function updateLine(n) {
                return n <= Math.min(to.line, to.line + lendiff)
                    ? n
                    : n + lendiff;
            }
            setSelection(
            ...
            code.style.height =
                doc.height * textHeight() + 2 * paddingTop() + "px";
        }
                    ...
                    if (change.to <= range.from && change.diff)
                        ...
                        if (change.to < range.to)
                            intact2.push({
                                ...
                                to: range.to + diff,
                                domStart:
                                    range.domStart + (change.to - range.from)
                            });
                    }
                }
                intact = intact2;
            ...
            doc.iter(from, to, function(line) {
                ...
                if (!nextIntact || nextIntact.from > j) {
                    if (line.hidden) scratch.innerHTML = "<pre></pre>";
                        ...
                        scratch.innerHTML = line.getHTML(
                            ch1,
                            ch2,
                            true,
                            tabText
        ...
        function setShift(val) {
            if (val)
                shiftSelecting =
                    shiftSelecting || (sel.inverted ? sel.to : sel.from);
        ...
        function indentLine(n, how) {
            ...
            if (!diff) {
                ...
                if (options.indentWithTabs)
                    for (
                        var i = Math.floor(indentation / options.tabSize);
                        i;
                        --i
                    ) {
                        pos += options.tabSize;
        ...
        function localCoords(pos, inLineWrap) {
            var x,
                ...
                y =
                    lh *
                    (heightAtLine(doc, pos.line) -
                        (inLineWrap ? displayOffset : 0));
            if (pos.ch == 0) x = 0;
            else {
        ...
        function coordsChar(x, y) {
            ...
            function getX(len) {
                ...
                if (tw) {
                    ...
                    return Math.max(
                        0,
                        sp.left + (off - innerOff) * scroller.clientWidth
                    );
                }
                return sp.left;
            }
        ...
        function endOperation() {
            ...
            if (
                ...
                !leaveInputAlone &&
                (updateInput === true ||
                    (updateInput !== false && selectionChanged))
            )
    ...
    CodeMirror.getMode = function(options, spec) {
        ...
        if (!mfactory) {
            if (window.console)
                console.warn(
                    "No mode " + mname + " found, falling back to plain text."
                );
    ...
    function lookupKey(name, extraMap, map) {
        ...
        return extraMap
            ? lookup(name, extraMap, map)
            : lookup(name, keyMap[map]);
    }
    ...
    Line.prototype = {
                    ...
                    if (!mark.from) {
                        for (var j = 0; j < mymk.length; ++j) {
                            ...
                            if (mymark.to == mylen && mymark.sameSet(mark)) {
                                mymark.to =
                                    mark.to == null ? null : mark.to + mylen;
                                if (mymark.isDead()) {
                                    mymark.detach(this);
                                    mk.splice(i--, 1);
                                }
        ...
        getHTML: function(sfrom, sto, includePre, tabText, endAt) {
            ...
            if (!allText && endAt == null)
                span(
                    " ",
                    sfrom != null && sto == null ? "CodeMirror-selected" : null
                );
