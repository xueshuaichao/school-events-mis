...
(function() {
    ...
    function initVideo() {
        ...
        (function() {
            ...
            if (img && img.className) {
                const hasFakedClass = img.className == "edui-faked-video";
                ...
                if (hasFakedClass || hasUploadClass) {
                    ...
                    const align = domUtils.getComputedStyle(img, "float");
                    const parentAlign = domUtils.getComputedStyle(
                        img.parentNode,
                        "text-align"
                    );
                    updateAlignButton(
    ...
    function updateAlignButton(align) {
        const aligns = $G("videoFloat").children;
        for (var i = 0, ci; (ci = aligns[i++]); ) {
            if (ci.getAttribute("name") == align) {
                if (ci.className != "focus") {
                    ci.className = "focus";
                }
            } else if (ci.className == "focus") {
                ci.className = "";
            }
        }
    }
    ...
    function insertSingle() {
        const width = $G("videoWidth");
        const height = $G("videoHeight");
        const url = $G("videoUrl").value;
        const align = findFocus("videoFloat", "name");
        if (!url) return false;
    ...
    function insertSearch(id) {
        const imgs = domUtils.getElementsByTagName($G(id), "img");
        const videoObjs = [];
    ...
    function switchSelect(selectParentId) {
        const selects = $G(selectParentId).children;
