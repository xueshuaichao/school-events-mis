...
function clickHandler(tabHeads, tabBodys, obj) {
    ...
    for (let j = 0, { length } = tabBodys; j < length; j++) {
...
function switchTab(tabParentId) {
    ...
    for (let i = 0, { length } = tabHeads; i < length; i++) {
...
$G("nextReplaceBtn").onclick = function(txt, dir, mcase) {
    const findtxt = $G("findtxt1").value;
    let obj;
    if (!findtxt) {
        return false;
    }
...
$G("repalceBtn").onclick = function() {
    const findtxt = $G("findtxt1").value.replace(/^\s|\s$/g, "");
    let obj;
