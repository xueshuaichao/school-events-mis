...
function addOkListener() {
    dialog.onok = function() {
        ...
        const urlPrefix = editor.getOpt("imageUrlPrefix");
        const images = domUtils.getElementsByTagName(editor.document, "img");
...
function showLocalPath(id) {
    ...
    const images = editor.execCommand("wordimage");
    if (images.length == 1 || (img && img.tagName == "IMG")) {
        g(id).value = images[0];
        return;
    }
    let path = images[0];
    const leftSlashIndex = path.lastIndexOf("/") || 0; // 不同版本的doc和浏览器都可能影响到这个符号，故直接判断两种
    const rightSlashIndex = path.lastIndexOf("\\") || 0;
    const separater = leftSlashIndex > rightSlashIndex ? "/" : "\\";
...
function createFlashUploader(opt, callbacks) {
    ...
    const i18n = utils.extend({}, lang.flashI18n);
    // 处理图片资源地址的编码，补全等问题
    for (const i in i18n) {
        if (
            !(
...
function createCopyButton(id, dataFrom) {
    ...
    const clipboard = baidu.swf.getMovie("copyFlash");
