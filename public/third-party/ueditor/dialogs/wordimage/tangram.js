...
baidu.swf.version = (function() {
    ...
    if (n.plugins && n.mimeTypes.length) {
        const plugin = n.plugins["Shockwave Flash"];
        ...
        for (let i = 12; i >= 2; i--) {
            try {
                const c = new ActiveXObject(
                    `ShockwaveFlash.ShockwaveFlash.${i}`
                );
                if (c) {
                    const version = c.GetVariable("$version");
...
baidu.swf.create = function(options, target) {
    options = options || {};
    const html = baidu.swf.createHTML(options) || options.errorMessage || "";
...
baidu.swf.getMovie = function(name) {
    const movie = document[name];
    let ret;
    return baidu.browser.ie == 9
        ? movie && movie.length
            ? (ret = baidu.array.remove(
                  baidu.lang.toArray(movie),
                  item => item.tagName.toLowerCase() != "embed"
              )).length == 1
                ? ret[0]
                : ret
            : movie
...
baidu.flash._Base = (function() {
    ...
    return function(options, callBack) {
        const me = this;
        const autoRender =
            typeof options.autoRender !== "undefined"
                ? options.autoRender
                : true;
        ...
        me.render = function() {
            ...
            if (callBack.length > 0) {
                baidu.each(callBack, (funName, index) => {
                    callBack[index] = _createFunName(
                        options[funName] || new Function()
                    );
                });
            }
...
baidu.flash.imageUploader =
    ...
    function(options) {
        const me = this;
        var options = options || {};
        const _flash = new baidu.flash._Base(options, [
            "selectFileCallback",
            "exceedFileCallback",
            "deleteFileCallback",
            "startUploadCallback",
...
baidu.flash.fileUploader =
    ...
    function(options) {
        ...
        me.deleteFile = function(index, callBack) {
            const callBackAll = function(list) {
...
baidu.sio.callByServer = /** @function */ function(url, callback, opt_options) {
    const scr = document.createElement("SCRIPT");
    const prefix = "bd__cbs__";
    let callbackName;
    let callbackImpl;
    const options = opt_options || {};
    const { charset } = options;
    const queryField = options.queryField || "callback";
    const timeOut = options.timeOut || 0;
    let timer;
    const reg = new RegExp(`(\\?|&)${queryField}=([^&]*)`);
    let matches;

    if (baidu.lang.isFunction(callback)) {
        callbackName =
            prefix + Math.floor(Math.random() * 2147483648).toString(36);
...
baidu.sio.log = function(url) {
    let img = new Image();
    const key = `tangram_sio_log_${Math.floor(
...
baidu.json.stringify = (function() {
    ...
    function encodeDate(source) {
        return `"${source.getFullYear()}-${pad(source.getMonth() + 1)}-${pad(
            source.getDate()
        )}T${pad(source.getHours())}:${pad(source.getMinutes())}:${pad(
            source.getSeconds()
        )}"`;
    }
